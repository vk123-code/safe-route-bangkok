...
export function calculateRiskWithDistricts(
  ...
  const school = liveDistricts.find((district) => district.name === schoolDistrictName);
  ...
  const floodAlertScore = 0;

  const score = rainfallScore + routeScore;
  ...
  const reasons = [
    ...
    home.trend === "worsening" || school.trend === "worsening"
      ? "Rainfall trend is worsening in at least one selected district."
      : "Rainfall trend is not currently worsening across the selected route.",
    "This version uses live Open-Meteo rainfall data only. It does not use fake flood alerts, fake sensors, or fake flood zones.",
  ];
